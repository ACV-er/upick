...
class ActivityController extends Controller
{
     ...
     *
     * @apiParam {String} activity_name  商家活动名称 长度40
     *
    ...
    {
        $offset = $request->route("page") * 15 - 15;

        $activity_list = Activity::query()->limit(7)->offset($offset)->orderByDesc("created_at")
    ...
    {
        $activity = Activity::query()->where("top", "1")->first()->get(["id", "activity_info", "top", "updated_at"]);
        if (!$activity) {
            return msg(3, "目标不存在" . __LINE__);
        }
